...
const SlidesScreen = () => {
  ...
  const scrollToSlide = (increment = 1) => {
    if (!flatListRef.current) return;
    if (currentSlideIndex + increment < 0) return;

    flatListRef.current.scrollToIndex({
      index: currentSlideIndex + increment,
      animated: true,
    });
  };
...
const SlideItem = ({ item }: SlideItemProps) => {
  ...
  return (
    ...
    >
      ...
      />
      <ThemedText className="text-light-primary dark:text-dark-background">
        {title}
      </ThemedText>
